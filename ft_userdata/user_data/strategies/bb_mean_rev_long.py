...
# isort: skip_file
"""
Bollinger Bands Mean Reversion Long Strategy for Freqtrade (USDT-perpetuals).
Long-only mean reversion:
- 15m close <= BB lower (20, 2)
- 15m RSI(14) < 35
- 4H close > 4H EMA20 (HTF trend filter)
Exits: Midden-BB exit via custom_exit, ROI ladder, Hard SL (-0.0631 = 0.75% equity risk), Trailing stop only after profit >= 1R.
"""
...
class bb_mean_rev_long(IStrategy):
    INTERFACE_VERSION = 3
    timeframe = "15m"
    can_short = False
    ...
    def populate_indicators(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        """Calculate 15m Bollinger Bands, RSI, and ATR."""
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Entry rules:
        # 1. 15m close <= BB lower (20, 2)
        bb_cond = dataframe["close"] <= dataframe["bb_lower"]
        # 2. 15m RSI(14) < 35
        rsi_cond = dataframe["rsi"] < 35.0
        # 3. 4H close > 4H EMA20 (higher timeframe bullish bias)
        htf_cond = dataframe["close_4h"] > dataframe["ema20_4h"]
        volume_ok = dataframe["volume"] > 0

        long_cond = bb_cond & rsi_cond & htf_cond & volume_ok

        dataframe.loc[long_cond, "enter_long"] = 1
        dataframe.loc[long_cond, "enter_tag"] = "bb_mean_rev_long_entry"
    ...
    def custom_exit(
        ...
        **kwargs,
        ...
        """Exit at middle BB (mean reversion target)."""
        ...
        # Exit when price reaches or crosses middle BB
    ...
    def custom_stoploss(
        ...
        **kwargs,
        ...
        Before 1R: trailing is inactive, relying on hard SL (-0.0631) + ROI ladder + BB mid exit.
        """
