...
class fib_poi_long_only(IStrategy):
    ...
    def populate_entry_trend(self, dataframe: DataFrame, metadata: dict) -> DataFrame:
        ...
        # Long only variant: shorts disabled
        dataframe.loc[:, "enter_short"] = 0
    ...
    def custom_stoploss(
        ...
        **kwargs,
        ...
        if not trade.is_short:
            # LONG stop: min(range_low, poi_long_lower) - 0.25 * ATR
            poi_bottom = float(last_row["poi_long_lower_4h"])
            r_low = float(last_row["range_low_4h"])
            base_stop = min(r_low, poi_bottom) - buffer_val

            # If TP1 hit, ensure stop is at least at break-even (open_rate)
            if trade.id in self.tp1_hit_trades:
                base_stop = max(base_stop, trade.open_rate)
            ...
            # SHORT stop: max(range_high, poi_short_upper) + 0.25 * ATR
            poi_top = float(last_row["poi_short_upper_4h"])
            r_high = float(last_row["range_high_4h"])
            base_stop = max(r_high, poi_top) + buffer_val

            # If TP1 hit, ensure stop is at least at break-even (open_rate)
            if trade.id in self.tp1_hit_trades:
                base_stop = min(base_stop, trade.open_rate)
